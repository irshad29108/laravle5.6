...
class TripReportRepository implements TripContract
{
    ...
    public function current($request, $client) {
        ...
        if ($httpResponse->getStatusCode() === 200) {
            $responseCollection = json_decode($httpResponse->getBody()->getContents())->response;
            $data = collect($responseCollection->data);
            $totalItems = $responseCollection->total;
            $pageNumber = isset($responseCollection->pageNo) ? $responseCollection->pageNo : 1;
        }
        ...
        if ($data->count()) {
            $response = $this->getTripReport($devices->slice(($currentPage-1) * $per_page, $per_page), $data, $totalItems, $pageNumber);
        }
    ...
    public function withFilters($request, $client){
     ...
     // GET TRIP REPORT
     private function getTripReport($devices, $data, $totalItems, $pageNo) {
        $response = new Collection;
        ...
        return $response->pad($totalItems, []);
    }
